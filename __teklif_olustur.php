    ...
    <div class="main-content">
        ...
        <div class="page-content">
            <h1>Yeni Teklif Oluştur</h1>
			            <!-- === YENİ: ŞABLON SEÇİM SATIRI === -->
            <div class="row mb-4">
                <div class="col-md-12">
                    <label for="template_id" class="form-label">Şablondan Yükle (İsteğe Bağlı)</label>
                    <select class="form-control" id="template_id">
                        <option></option> <!-- Select2 placeholder için -->
                        <?php foreach($templates as $template): ?>
                            <option value="<?php echo $template['id']; ?>"><?php echo htmlspecialchars($template['text']); ?></option>
                        <?php endforeach; ?>
                    </select>
...
<!-- === DEĞİŞKENLERİ JAVASCRIPT'E AKTARMA === -->
<script>
    // PHP dizilerini JavaScript değişkenleri olarak tanımlıyoruz
    var customersData = <?php echo json_encode($customers ?? []); ?>;
    var templatesData = <?php echo json_encode($templates ?? []); ?>;
    var mevcutTeklif = <?php echo json_encode($teklif ?? null); ?>; // Revizyon sayfası için
</script>
